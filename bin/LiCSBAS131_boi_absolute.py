...
to the residual cumulative displacement time-series (`cum`) of sbovl
generated by LiCSBAS. The aim is to convert the relative sbovl time series into an absolute reference frame.
...
This will:
 ...
 - Interpolate `daz` to match the time steps of the sbovl time-series cube
...
#%% Change log
'''
20260518 Muhammet Nergizci, COMET University of Leeds
- Ransac debug compatible with new version of scikit-learn.
...
def main(argv=None):
    ...
    # Drop extreme daz outliers, is needed for both plotting. MN
    daz_limit = 1000  # mm

    bad_daz = df_daz['daz'].abs() > daz_limit

    if bad_daz.any():
        print("\nDropping extreme DAZ outliers:")
        print(df_daz.loc[bad_daz, ['epoch', 'daz']])

    df_daz.loc[bad_daz, 'daz'] = np.nan

    # Refill dropped values using nearest/interpolation
    df_daz['daz'] = df_daz['daz'].interpolate(method='linear', limit_direction='both')
    ...
    if np.sum(mask) < 2:
        ...
        reg = RANSACRegressor(estimator=LinearRegression()).fit(X[mask], y[mask])
