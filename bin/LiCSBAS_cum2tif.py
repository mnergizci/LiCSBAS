...
def cum_tiff_png(TS_folder, cum_h5, mask, dem_par, frame_number, imd_s, plate_motion=False):
  breakpoint()
  ...
  # breakpoint()
  os.system(f"LiCSBAS_cum2flt.py -i {cum_h5} -d {imd_s} -o {TS_folder}/results/{cum_name}_{imd_s}.mask --mask {mask}")
  if plate_motion:
    print('Cumulated plate motion effect will be removed from the cumulative displacement.')
    os.system(f"LiCSBAS_vel_plate_motion.py -t {TS_folder} -f {frame_number} {cum_h5} -d {imd_s} -o {TS_folder}/results/{cum_name}_{imd_s}.mask --mask {mask}")
  else:  
    os.system(f"LiCSBAS_flt2geotiff.py -i {TS_folder}/results/{cum_name}_{imd_s}.mask -p {dem_par} -o {frame_number}.{cum_name}_{imd_s}.mask.tif")
    os.system(f"LiCSBAS_disp_img.py -i {TS_folder}/results/{cum_name}_{imd_s}.mask -p {dem_par} --cmin -100 --cmax 100 --png {frame_number}.{cum_name}_{imd_s}.mask.png --title {frame_number}.{cum_name}_{imd_s}.mask")
...
if __name__ == "__main__":
    ...
    cum_tiff_png(args.TS_folder ,args.cum_h5, args.mask, args.dem_par, args.frame_number, args.imd_s, args.plate_motion)
